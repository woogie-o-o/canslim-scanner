...
def get_fear_greed(force: bool = False) -> dict:
    """CNN Fear & Greed Index와 최근 히스토리. 실패 시 stale 캐시 또는 빈 값."""
    global _FG_CACHE, _FG_CACHE_TS
    import json as _json

    now = time.time()
    with _FG_LOCK:
        if _FG_CACHE and not force and now - _FG_CACHE_TS < 15 * 60:
    ...
    try:
        ...
        history = []
        for item in raw_history[-90:]:
            try:
                history.append({"x": int(item["x"]), "y": round(float(item["y"]), 1)})
            except Exception:
                continue
